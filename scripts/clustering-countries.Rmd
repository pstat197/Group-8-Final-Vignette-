...
To define such a distribution for a given set of observations, we need to define a covariance matrix. We won't get into the mathematical details here (see the paper here for a more rigorous treatment), but the basic idea is that by modifying this matrix, we can create different types of clusters that are determined by three things:
...
Below is a table containing all of the possible types of clusters. The identifiers are made up of three letters, which indicate what volume, shape, and orientation the clusters have. These are labeled as follows:
